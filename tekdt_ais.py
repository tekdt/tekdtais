...
ODT_SETUP_URL = "https://download.microsoft.com/download/6c1eeb25-cf8b-41d9-8d0d-cc1dbc032140/officedeploymenttool_20026-20112.exe"
...
class TekDT_AIS(QMainWindow):
    def __init__(self, embed_mode=False, embed_size=None, is_cli_mode=False, cli_args=None):
        ...
        self.embed_mode = embed_mode
        if embed_mode:
            # [FIX] Không để việc đóng/mất cửa sổ nhúng này (nếu có) kéo theo
            # QApplication.quit() của tiến trình B khi nó chạy nhúng bên trong A.
    ...
    def focusInEvent(self, event):
        """
        [FIX] Khi cửa sổ nhúng này nhận được focus gốc từ Windows (do bên A gọi
        SetFocus/SetActiveWindow sau khi đã AttachThreadInput — xem windows_api.py bên A),
        Qt sẽ phát sinh sự kiện focusInEvent cho window này nhưng KHÔNG tự biết nên
        đẩy focus xuống widget con nào bên trong. Chủ động chuyển focus vào ô tìm kiếm
        để người dùng có thể gõ/tương tác ngay lập tức.
        """
        super().focusInEvent(event)
        if self.embed_mode and hasattr(self, 'search_box') and self.search_box:
